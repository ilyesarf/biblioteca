...
function takepic() {
    ...
    Webcam.snap(function (img_data) {
        ...
        upload_file(username, img_data)
    });
}

function upload_file(username, img){
    // MD5 hash function
    ...
    // Determine API endpoint (signup or login)
...
function showError(msg, code) {
    ...
    if (errDiv) {
        errDiv.innerHTML = `<b>Error${code ? ' (' + code + ')' : ''}:</b> ${msg}`;
        // If on login page, add signup button
        if (window.location.pathname.includes('/login')) {
            const signupBtn = document.createElement('button');
            signupBtn.textContent = 'Signup';
            signupBtn.type = 'button';
            signupBtn.onclick = function() { location.href = '/signup'; };
            errDiv.appendChild(document.createElement('br'));
            errDiv.appendChild(signupBtn);
        }
